...
var FileTrieNode = class _FileTrieNode {
  ...
  get slug() {
    const path10 = joinSegments(...this.slugSegments);
    if (this.isFolder) {
      return joinSegments(path10, "index");
    }
    return path10;
  }
  get slugSegment() {
  ...
  makeChild(path10, file) {
    const fullPath = [...this.slugSegments, path10[0]];
  ...
  insert(path10, file) {
    ...
    this.isFolder = true;
    const segment = path10[0];
    if (path10.length === 1) {
      if (segment === "index") {
        this.data ??= file;
      } else {
        this.makeChild(path10, file);
      }
    } else if (path10.length > 1) {
      const child = this.children.find((c) => c.slugSegment === segment) ?? this.makeChild(path10, void 0);
      const fileParts = file.filePath.split("/");
      child.fileSegmentHint = fileParts.at(-path10.length);
      child.insert(path10.slice(1), file);
    }
  }
  ...
  findNode(path10) {
    if (path10.length === 0 || path10.length === 1 && path10[0] === "index") {
      return this;
    }
    return this.children.find((c) => c.slugSegment === path10[0])?.findNode(path10.slice(1));
  }
  ancestryChain(path10) {
    if (path10.length === 0 || path10.length === 1 && path10[0] === "index") {
      return [this];
    }
    const child = this.children.find((c) => c.slugSegment === path10[0]);
    if (!child) {
      return void 0;
    }
    const childPath = child.ancestryChain(path10.slice(1));
    if (!childPath) {
      return void 0;
    }
   ...
   */
  getFolderPaths() {
    return this.entries().filter(([_, node]) => node.isFolder).map(([path10, _]) => path10);
  }
};
...
var Head_default = /* @__PURE__ */ __name((() => {
  const Head = /* @__PURE__ */ __name(({
    ...
    const baseDir = fileData.slug === "404" ? path10 : pathToRoot(fileData.slug);
...
function toPosixPath(fp) {
  return fp.split(path7.sep).join("/");
}
...
var Assets = /* @__PURE__ */ __name(() => {
  return {
    ...
    async *partialEmit(ctx, _content, _resources, changeEvents) {
      for (const changeEvent of changeEvents) {
        const ext = path8.extname(changeEvent.path);
        if (ext === ".md") continue;
        if (changeEvent.type === "add" || changeEvent.type === "change") {
...
var NotFoundPage = /* @__PURE__ */ __name(() => {
  ...
  return {
    ...
    async *emit(ctx, _content, resources) {
      ...
      const externalResources = pageResources(path10, resources);
      const componentData = {
        ctx,
        fileData: vfile.data,
        externalResources,
  ...
  return async (processor) => {
    ...
    for (const fp of fps) {
      try {
        ...
        file.data.relativePath = path9.posix.relative(argv.directory, file.path);
